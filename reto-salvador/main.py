...
for place in root:
    id = place.attrib['place_id']
    for i in range(len(gasolinerias)):
        if(id in gasolinerias[i]['id']):
            for child in place:
                type = child.attrib['type']
                if(type == 'regular'):
                    gasolinerias[i]['regular'] = child.text
                if(type == 'premium'):
                    gasolinerias[i]['premium'] = child.text
                if(type == 'diesel'):
                    gasolinerias[i]['diesel'] = child.text
                

f = open('data.csv', 'a', newline='')

writer = csv.writer(f) 
writer.writerow(('nombre', 'regular', 'premium', 'diesel','ciudad','pais','codigo postal'))
for items in gasolinerias:
    if 'diesel' in items:
        writer.writerow((items['name'], items['regular'], items['premium'], items['diesel'], items['city'], items['country'], items['postcode']))
    else:
        writer.writerow((items['name'], items['regular'], items['premium'], '', items['city'], items['country'], items['postcode']))
f.close()
